...
def reward_model_lpips_l1_batch(
    ...
    l1 = (gen_01 - gt_01).abs().mean(dim=(1, 2, 3))    # (N,)
...
def euler_sde_step_with_logprob(
    ...
    #################### mean formulation ##################
#     sq_err = ((x_next - mean).pow(2) / std.pow(2).clamp(min=1e-12)).mean(dim=(1, 2, 3))  # (B,)
#     log_std_b = std.reshape(B).log()  # (B,)
...
class GRPOConfig(TrainConfig):
    ...
    clip_range: float = 1e-3       # PPO clipping range for ratio
    ...
    num_inner_epochs: int = 5       # Inner training epochs over collected batch
    ...
    wandb_run_name: str = "grpo-run-l2-seed4-debug"
    save_suffix: str = f"grpo-run-l2-seed4-debug"
