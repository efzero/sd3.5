...
class SD3:
    def __init__(self, model_path: str, shift: float, device: str = "cuda", dtype: torch.dtype = torch.bfloat16): ###change back to torch.float32 if loss diverges
...
class VAE:
    def __init__(self, model_path: str, device: str = "cuda", dtype: torch.dtype = torch.float32):
        with safe_open(model_path, framework="pt", device="cpu") as f:
...
#     model: str = "models/sd3.5_medium.safetensors"
    model: str="models/sd3.5_large.safetensors"
    ...
    sample_steps: int = 40  # Sampling steps (fewer for faster sampling)
...
class SD3Trainer:
    ...
    def load_models(self):
        ...
        #######test whether torch.float16 works
...
#             cond_c, cond_y = self.get_cond_image(lq_images, captions)
        ...
        for k in range(batch_size):
            if lq_images[k] == "":
                cond_c, cond_y = self.encode_prompts([captions[k]])
                cond_cs.append(cond_c)
                cond_ys.append(cond_y)
            else:
                cond_c, cond_y = self.get_cond_image([lq_images[k]], [captions[k]])
                cond_cs.append(cond_c)
                cond_ys.append(cond_y)
        cond_c, cond_y = torch.cat(cond_cs, dim = 0), torch.cat(cond_ys, dim = 0)
...
#         params = [] ####no LORA
    ...
    def train(
        ...
        global_step = 0          # "optimizer step" counter (updates)
        start_epoch = 0

        # -------------------------
        # Prompt picking (unchanged)
        # -------------------------
        first_prompt = None
        if len(dataset) > 0:
        ...
        # ============================
        grad_accum_steps = max(1, int(self.config.grad_accum_steps))  ### [NEW]
        micro_step = 0                                               ### [NEW]
        optimizer.zero_grad(set_to_none=True)                         ### [NEW] start clean

        for epoch in range(start_epoch, self.config.epochs):
            for batch_idx, (images, lq_images, captions) in enumerate(dataloader):
                micro_step += 1                                      ### [NEW] counts microbatches

                captions = list(captions)
                lq_images = list(lq_images)

                # caption / cond drop (unchanged)
                for k in range(len(captions)):
                    drop_prob = random.random()
                    if drop_prob < 0.3:
                        captions[k] = ""
                    elif 0.3 <= drop_prob < 0.5:
                        captions[k] = ""
                        lq_images[k] = ""
                ...
                # -----------------------------------------
                # Accumulation: scale loss then backward
                # -----------------------------------------
                true_loss = loss.detach()                             ### [NEW] keep for logging
                (loss / grad_accum_steps).backward()                  ### [CHANGED] do division here

                # Optional: log microbatch loss (not optimizer-step loss)
#                 wandb.log({"train/loss_micro": float(true_loss.item())}, step=micro_step)  ### [NEW]

                # -----------------------------------------
                # Do optimizer step only every N microbatches
                # -----------------------------------------
                if (micro_step % grad_accum_steps) == 0:              ### [CHANGED] use micro_step
                    global_step += 1                                  ### [CHANGED] now means "update step"

                    # Clip gradients for all trainable parameters (if desired)
                    ...
                    if getattr(self, "clip_vision_adaptor", None) is not None:
                        all_params.extend([p for p in self.clip_vision_adaptor.parameters() if p.requires_grad])
                    ...
                    # Log the "effective" loss per update
                    print(true_loss.item(), global_step, "global step (update)")            ### [CHANGED]
                    wandb.log({"train/loss": float(true_loss.item())}, step=global_step)   ### [CHANGED]

                    if global_step % self.config.log_every == 0:
                        print(f"[epoch {epoch} step {global_step}] loss={true_loss.item():.6f}")  ### [CHANGED]
                    ...
                    if global_step % self.config.save_every == 0:
                        ckpt_path = os.path.join(
                            self.config.out_dir,
                            f"sd3_large_step_{global_step}_lqmedium_rank32_uniformsigma_nullembed.pt"
                        )
        ...
        # Final save (unchanged except step meaning)
...
#                 neg_cond_c, neg_cond_y = self.get_cond_image([cond_im], [""]) ####uncond with image embedding
            ...
            if save_name is None:
                sample_path = os.path.join(
                    ...
                    f"sample_step_{step:06d}_{seed_offset}_recon_{im_num}_new_lora32_cfglrvanilla_sd3large_nullembed_uniform.png",
                )
            else:
